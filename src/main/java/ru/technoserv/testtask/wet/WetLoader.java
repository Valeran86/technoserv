...
/** Facade for using WARC parser
 * Created by Home on 28.06.2017.
 */
public class WetLoader {
    public static final String DEFAULT_URL = "crawl-data/CC-MAIN-2017-17/segments/1492917118310.2/wet/CC-MAIN-20170423031158-00013-ip-10-145-167-34.ec2.internal.warc.wet.gz";
    private static final String WET_DATA_BASE = "wet.paths.gz";
    private static final String ENCODING = "utf8";

    /** Generate wet sample url for task */
    public static String generateWet() {
        String wetUrl;
        try {
    ...
    public static List<Page> getPages(String wetUrl) {
        //point to use a DB for next queries but if use it - search should be in DB level too
        //savePages2DB(pages);

        Pattern urlStart = Pattern.compile("^https?://", Pattern.CASE_INSENSITIVE);

        List<Page> pages = new LinkedList<>();
        ArchiveReader ar = getArchiveReader(wetUrl);
